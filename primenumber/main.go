...
// this signature with io.Reader is only for test :(
...
func isPrime(n int) (bool, string) {
	...
	if n < 0 {
		return false, fmt.Sprintf("Negative numbers are not prime, by defition!")
	}

	// use the modulus operator repeatedly to see if we a prime number
	for i := 2; i <= n/2; i++ {
		if n%i == 0 {
